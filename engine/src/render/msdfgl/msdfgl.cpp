...
#include "render/msdfgl/msdfgl.hpp"
#include "render/msdfgl/msdfgl_map.hpp"
#include "render/msdfgl/msdfgl_serializer.hpp"
...
int compile_shader(const char *source, GLenum type, GLuint *shader, const char *version)
{
  ...
  *shader = glCreateShader(type);
  if (!*shader) { fprintf(stderr, "failed to create shader\n"); }
...
msdfgl_font_t
  _msdfgl_init_font_internal(msdfgl_context_t ctx, FT_Face *face, float range, float scale, msdfgl_atlas_t atlas)
{
...
{
  FT_Face face;
  if (FT_New_Face(ctx->ft_library, font_name, 0, &face)) { return NULL; }

  return _msdfgl_init_font_internal(ctx, &face, range, scale, atlas);
}

MSDFGL_EXPORT msdfgl_font_t msdfgl_load_font_mem(msdfgl_context_t ctx,
  void *font_buffer,
...
{
  FT_Face face;
  if (FT_New_Memory_Face(ctx->ft_library, font_buffer, font_buffer_size, 0, &face)) { return NULL; }
...
int _msdfgl_generate_glyphs_internal(msdfgl_font_t font,
  int32_t start,
  int32_t end,
  unsigned int range,
  int32_t *keys,
  int nkeys)
{
  ...
  if (!atlas->nglyphs && range && !start) { font->_direct_lookup_upper_limit = end; }
  ...
  for (size_t i = 0; (int)i < (int)nrender; ++i) {
    ...
    while ((atlas->offset_y + buffer_height) > new_texture_height) { new_texture_height *= 2; }
    if (new_texture_height > font->context->_max_texture_size) { goto error; }
    while ((int)(atlas->nglyphs + i) >= new_index_size) { new_index_size *= 2; }
  }
  ...
  if ((int)atlas->nallocated == new_index_size) {
    ...
    if (atlas->nglyphs) {
      ...
      glCopyBufferSubData(GL_COPY_READ_BUFFER, GL_ARRAY_BUFFER, 0, 0, atlas->nglyphs * sizeof(msdfgl_index_entry));
  ...
  if (atlas->texture_height == new_texture_height) {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA32F, atlas->texture_width, new_texture_height, 0, GL_RGBA, GL_FLOAT, NULL);
  ...
  _msdfgl_ortho(0, (GLfloat)atlas->texture_width, 0, (GLfloat)atlas->texture_height, -1.0, 1.0, framebuffer_projection);
  _msdfgl_ortho(-(GLfloat)atlas->texture_width,
    (GLfloat)atlas->texture_width,
    -(GLfloat)atlas->texture_height,
    (GLfloat)atlas->texture_height,
    -1.0,
    1.0,
    atlas->projection);
  ...
  for (int i = 0; i < nrender; ++i) {
    ...
    glUniform2f(ctx->_translate_uniform,
      -g.bearing_x / SERIALIZER_SCALE + font->range / 2.0f,
...
void msdfgl_render(msdfgl_font_t font, msdfgl_glyph_t *glyphs, int n, GLfloat *projection)
{
  ...
  glVertexAttribPointer(
    0, 2, GL_FLOAT, GL_FALSE, sizeof(struct _msdfgl_glyph), (void *)offsetof(struct _msdfgl_glyph, x));

  glEnableVertexAttribArray(1);
  glVertexAttribIPointer(
    1, 4, GL_UNSIGNED_BYTE, sizeof(struct _msdfgl_glyph), (void *)offsetof(struct _msdfgl_glyph, color));
  ...
  glVertexAttribPointer(
    3, 1, GL_FLOAT, GL_FALSE, sizeof(struct _msdfgl_glyph), (void *)offsetof(struct _msdfgl_glyph, size));

  glEnableVertexAttribArray(4);
  glVertexAttribPointer(
    4, 1, GL_FLOAT, GL_FALSE, sizeof(struct _msdfgl_glyph), (void *)offsetof(struct _msdfgl_glyph, offset));

  glEnableVertexAttribArray(5);
  glVertexAttribPointer(
    5, 1, GL_FLOAT, GL_FALSE, sizeof(struct _msdfgl_glyph), (void *)offsetof(struct _msdfgl_glyph, skew));

  glEnableVertexAttribArray(6);
  glVertexAttribPointer(
    6, 1, GL_FLOAT, GL_FALSE, sizeof(struct _msdfgl_glyph), (void *)offsetof(struct _msdfgl_glyph, strength));
  ...
  for (size_t i = 0; buf_idx < bufsize; ++i) {
    ...
    if (flags & MSDFGL_KERNING && (prev_key != -1) && FT_HAS_KERNING(font->face))
      FT_Get_Kerning(font->face,
        FT_Get_Char_Index(font->face, prev_key),
