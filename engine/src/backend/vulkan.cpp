...
namespace Anterle {
VulkanBackend::VulkanBackend(int width, int height, const char *window_name) : BaseBackend(width, height, window_name)
{
...
void VulkanBackend::PostRender()
{
  ...
  if (!is_minimized) {
    g_MainWindowData.ClearValue.color.float32[0] = 0.45f * 1.0f;
    g_MainWindowData.ClearValue.color.float32[1] = 0.55f * 1.0f;
    g_MainWindowData.ClearValue.color.float32[2] = 0.60f * 1.0f;
