...
const std::string TEXTURE_PATH = "resources/texture/viking.png";
const std::string MODEL_PATH = "resources/model/viking.obj";
...
void VulkanBackend::PostRender() { vkDeviceWaitIdle(device); }
...
void VulkanBackend::createImageViews()
{
  ...
  for (uint32_t i = 0; i < swapChainImages.size(); i++) {
    swapChainImageViews[i] =
      createImageView(device, swapChainImages[i], swapChainImageFormat, VK_IMAGE_ASPECT_COLOR_BIT, 1);
  }
}
void VulkanBackend::createColorResources()
{
...
void VulkanBackend::updateUniformBuffer(uint32_t currentImage)
{
  ...
  ubo.proj = glm::perspective(
    glm::radians(45.0f), static_cast<float>(swapChainExtent.width / swapChainExtent.height), 0.1f, 10.0f);
  ubo.proj[1][1] *= -1;
...
{
  std::vector<char> vertShaderCode = readFile("resources/shader/base.vert.spv");
  std::vector<char> fragShaderCode = readFile("resources/shader/base.frag.spv");
...
void VulkanBackend::createTextureImageView()
{
  textureImageView =
    createImageView(device, textureImage, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_ASPECT_COLOR_BIT, mipLevels);
}
